...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5 ">
            <div class="fs-1 fw-bold mb-4 text-center">ประกาศ EGP</div>
            ...
            <table class="table">
                @foreach($Egp as $detail)
                <tr>
                    <td><a href="{{route('EGPDetail',$detail->id)}}">{{ $detail->title_name }}</a></td>
                </tr>
                @endforeach
            </table>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mt-5">
                    <!-- Previous button -->
                    <li class="page-item {{ $Egp->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $Egp->previousPageUrl() }}">ก่อนหน้า</a>
                    </li>

                    <!-- Page number buttons -->
                    @foreach ($Egp->getUrlRange(1, $Egp->lastPage()) as $page => $url)
                    <li class="page-item {{ $page == $Egp->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                    </li>
                    @endforeach

                    <!-- Next button -->
                    <li class="page-item {{ !$Egp->hasMorePages() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $Egp->nextPageUrl() }}">ต่อไป</a>
                    </li>
                </ul>
            </nav>
            @endif
