...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center">การประเมินประสิทธิภาพภายใน (LPA) <br><span class="fs-3">{{$listDetail->list_details_name}}</div>

            <p class="text-muted">วันที่เผยแพร่: {{ \Carbon\Carbon::parse($listDetail->created_at)->format('d-m-Y') }}</p>

            <!-- ไฟล์แนบ: PDF -->
            @if ($listDetail->pdf && $listDetail->pdf->count() > 0)
            <h5 class="text-secondary mt-4">ไฟล์เอกสาร</h5>
            @foreach ($listDetail->pdf as $pdf)
            <div class="mb-3">
                <iframe src="{{ asset('storage/' . $pdf->pdf_file) }}" width="100%" height="700px"></iframe>
            </div>
            @endforeach
            @endif
