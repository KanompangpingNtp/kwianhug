...
@section('title', 'ข้อมูลชุมชน')
@section('content')

<h2 class="text-center">จัดการข้อมูลชุมชน</h2><br>

<div class="card mt-3">
    <div class="card-body">
        @forelse ($basicInfoDetail as $detail)
        <form action="{{ route('CommunityDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้หรือไม่?');">
            @csrf
            @method('DELETE')
        ...
        <form action="{{ route('CommunityCreate') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div>
                <input type="hidden" name="basic_info_type" value="{{ $basicInfoType->firstWhere('type_name', 'ข้อมูลชุมชน')->id }}">
            </div>
