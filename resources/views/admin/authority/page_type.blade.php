...
<h3 class="text-center">สร้างหัวข้ออำนาจหน้าที่</h3><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('AuthorityTypeCreate') }}" method="POST">
                @csrf
...
<table class="table table-bordered" id="data_table">
    ...
    <tbody>
        @foreach ($AuthorityType as $index => $details)
        <tr>
            <td class="text-center">{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('AuthorityShowDetail', $details->id) }}">
                    {{ $details->type_name }}
                </a>
            </td>
            <td class="text-center">
                ...
                <form action="{{ route('AuthorityTypeDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
...
@foreach ($AuthorityType as $details)
<div class="modal fade" id="editModal-{{ $details->id }}" tabindex="-1" aria-labelledby="editModalLabel-{{ $details->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('AuthorityTypeUpdate', $details->id) }}" method="POST">
                @csrf
                @method('PUT')
