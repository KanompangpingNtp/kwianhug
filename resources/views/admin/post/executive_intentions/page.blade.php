...
@section('title', 'เจตจำนงสุจริตของผู้บริหาร')
@section('content')

<h2 class="text-center">เจตจำนงสุจริตของผู้บริหาร</h2><br>

<div class="card mt-3">
    <div class="card-body">
        @forelse ($basicInfoDetail as $detail)
        <form action="{{ route('ExecutiveIntentionsDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้หรือไม่?');">
            @csrf
            @method('DELETE')
        ...
        <form action="{{ route('ExecutiveIntentionsCreate') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div>
                <input type="hidden" name="basic_info_type" value="{{ $basicInfoType->firstWhere('type_name', 'เจตจำนงสุจริตของผู้บริหาร')->id }}">
            </div>
