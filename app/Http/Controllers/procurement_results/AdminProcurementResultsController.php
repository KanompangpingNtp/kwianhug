...
class AdminProcurementResultsController extends Controller
{
    public function ProcurementResultsHome()
    {
        $postTypes = PostType::all();

        $postTypeId = $postTypes->firstWhere('type_name', 'ผลประกาศจัดซื้อจัดจ้าง')->id;
        $postDetails = PostDetail::with('postType', 'pdfs')
            ->where('post_type_id', $postTypeId)
            ->orderBy('created_at', 'desc')
            ->get();

        return view('admin.post.procurement_results.page', compact('postDetails', 'postTypes'));
    }
    ...
    public function ProcurementResultsCreate(Request $request)
    {
        ...
        if ($request->hasFile('file_post')) {
            foreach ($request->file('file_post') as $file) {
                ...
                $path = $file->storeAs('pdf/procurement_results', $filename, 'public');
