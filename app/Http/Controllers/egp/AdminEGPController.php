...
class AdminEGPController extends Controller
{
    public function EGPHome()
    {
        $postTypes = PostType::all();

        $postTypeId = $postTypes->firstWhere('type_name', 'egp')->id;
        $postDetails = PostDetail::with('postType', 'pdfs')
            ->where('post_type_id', $postTypeId)
            ->orderBy('date', 'desc')
            ->get();

        return view('admin.post.egp.page', compact('postDetails', 'postTypes'));
    }

    public function EGPCreate(Request $request)
    {
        $request->validate([
    ...
    public function EGPUpdate(Request $request, $id)
    {
        $request->validate([
            'date' => 'nullable|date',
            'title_name' => 'nullable|string',
